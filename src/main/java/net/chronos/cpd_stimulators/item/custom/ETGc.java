...
//            level.playSound(player, player.getOnPos(), ModSounds.APPLY_INJECTOR.get(), SoundSource.PLAYERS, 1.0f, 1.0f);
    ...
    private void addSideEffects(Player player) {
        ModClientTickEvent.queueServerWork(1900, () -> {
            player.addEffect(new MobEffectInstance(MobEffects.HUNGER, 400));
            player.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, 1200));
            player.addEffect(new MobEffectInstance(ModEffects.EXHAUSTION.getDelegate(), 1200));
        });
    }
